/**
 * A2A payment workflow: intent → quote → confirm → settle (paper/sim default).
 */
...
export class PayPulseEngine {
  ...
  constructor(opts?: { facade?: AgentOsFacade; agents?: AgentProfile[] }) {
    this.facade = opts?.facade ?? new AgentOsFacade({ mode: "paper" });
  ...
  async runPayment(req: CreatePaymentRequest): Promise<WorkflowResult> {
    ...
    if (
      ...
      record = {
        ...
        stage: "settle",
        updatedAt: now,
        rejectReason: settleRes.data.note,
        usedMock: settleRes.usedMock,
        mockLabel: settleRes.label,
      };
    }
...
/** Demo scenario: 2 successful paper A2A + 1 rejected over cap */
