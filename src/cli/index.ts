...
async function main(): Promise<void> {
  ...
  const engine = new PayPulseEngine({ facade: new AgentOsFacade({ mode: "paper" }) });

  if (cmd === "once") {
    const r = await engine.payForService("agent-buyer-alpha", "svc-market-brief", {
      confirm: true,
    });
    console.log(JSON.stringify({ payment: r.payment, settlement: r.settlement, adapterMeta: r.adapterMeta }, null, 2));
    return;
  }
